import pytest 
import numpy as np
from CCLASSY import LogisticRegressionClassifier
...
def test_invalid_n_iterations_negative():
    with pytest.raises(ValueError):
        LogisticRegressionClassifier(n_iterations=-1)
...
def test_invalid_learning_rate_negative():
    with pytest.raises(ValueError):
        LogisticRegressionClassifier(learning_rate=-0.1)
...
def test_invalid_X_not_2d(simple_data):
    _, y = simple_data
    with pytest.raises(ValueError):
        LogisticRegressionClassifier().fit(np.array([1.0, 2.0, 3.0]), y)
        
def test_invalid_y_not_binary(simple_data):
    X, _ = simple_data
    with pytest.raises(ValueError):
        LogisticRegressionClassifier().fit(X, np.array([0, 1,0]))
        
        
def test_predict_before_fit():
    with pytest.raises(RuntimeError):
        LogisticRegressionClassifier().predict(np.array([[1.0, 2.0]]))
        
def test_predict_correct_classes(simple_data):
    X_train, y_train = simple_data
    X_test = np.array([[1.1, 2.0], [5.1, 6.0]])
    clf = LogisticRegressionClassifier().fit(X_train, y_train)
...
def test_refit(simple_data):
    X,y = simple_data
    clf = LogisticRegressionClassifier().fit(X, y).fit(X,y)
    assert clf.fitted == True
